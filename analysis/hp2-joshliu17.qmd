...
But what about accuracy versus frequency?
...
The accuracy rating for the first 2000 or so most frequent characters seems to be constant, floating around the 1.00 range, but seems to drop off as characters become less frequent. Perhaps there is a reason certain characters are more frequent than others. 

The Chinese Language Database (Frequency) provides lots of useful lexical properties of characters such as radical, semantic meaning, stroke count, and tone. This information is especially helpful when paired with the data gathered from the Chinese Lexicon Project. We have found that less common characters resulted in slower response times in the study. Furhter, perhaps the complexity of a character also influences respondent's response times. 

```{r}

combined_data %>%
  filter(Stroke_Count > "5") %>%
  ggplot(aes(x = Stroke_Count, y = Frequency)) +
  geom_point()
